...
export default function RankingPage() {
  ...
  const rest = page === 1 ? butecos.slice(3) : butecos;
  ...
  borderColor: string
    ...
    <div
      className={`rounded-xl p-4 w-28 md:w-48 text-center shadow-md border ${bgColor} ${borderColor}`}
    >
  ...
  return (
    <div className="space-y-8">
      ...
      {top3 && (
        <section aria-labelledby='pódio' className={page !== 1 ? 'hidden' : ''}>
        ...
        {rest.map((buteco, idx) => (
          <div key={buteco.id} className="p-4 border rounded-lg flex justify-between items-center">
            <div className="flex items-center gap-4">
              <span className="text-xl font-bold">{page === 1 ? idx + 4 : idx + 1 + (page - 1) * 10}</span>
